...
class WeWork_Send_Msg():
    def __init__(self):
        self.send_url = "https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=8614fc60-5cc7-41ad-8201-94c605f672d1"

    # 文本类型消息
    def send_txt(self):
        headers = {"Content-Type": "text/plain"}
        #李晓良测试企业微信
        #send_url = "https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=8614fc60-5cc7-41ad-8201-94c605f672d1"
        send_data = {
            "msgtype": "text",  # 消息类型
            "text": {
                "content": "深圳今日天气：32度，暴雨",  # 文本内容，最长不超过2048个字节，必须是utf8编码
                "mentioned_list": ["@all"], #userid 列表，提醒群中的指定成员，@all表示提醒所有人
                # userid的列表，提醒群中的指定成员(@某个成员)，@all表示提醒所有人，如果开发者获取不到userid，可以使用mentioned_mobile_list
                "mentioned_mobile_list": ["17606070429"]  # 手机号列表，提醒手机号对应的群成员(@某个成员)，@all表示提醒所有人
            }
        }

        res = requests.post(url=self.send_url, headers=headers, json=send_data)
        print(res.text)
...
if __name__ =="__main__":
    we_test = WeWork_Send_Msg()
    we_test.send_text_img()